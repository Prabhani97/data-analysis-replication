...
1.  First, descriptive statistics are used to summarize participant characteristics and key predictor variables for the ONH and OHI groups. This includes variables such as age, hearing sensitivity, working memory, vocabulary, and speech glimpsing ability.

2.  Second, a descriptive visualization is created using violin plots to compare the distributions of key predictor variables between groups, similar to Fig. 3 in the original study.

3.  Third, an inferential analysis is conducted to examine the relationship between predictor variables and speech recognition outcomes, providing a simplified approximation of the dominance analysis used in the original study.I will do a visualzation of this analysis too.

4.  Finally, a correlation analysis is performed to explore relationships between auditory and cognitive predictors and speech recognition performance, and to assess whether similar patterns to those reported in the original study are observed.
...
The first descriptive analysis in the original paper focuses on describing the demographic and baseline characteristics of the participants. This includes summarizing age, gender distribution, hearing thresholds, and key predictor variables for the two groups: older normal-hearing (ONH) and older hearing-impaired (OHI) listeners.
...
Now I am loading the correct sheet where the data after doing the PCA.
...
Key predictor variables : Audiogram_PCA ; Glimpsing ; WorkingMemory_PCA; Vocabulary. The authors have not generated any kind of summarization or any descriptive statistics out of these variables. But, they have analyzed a frequecny by frequency mean audiogram from raw threshold columns like 250Hz, 500Hz, 1000Hz and so on from the "AuditoryVariables" sheet.
...
# Keep only audiogram frequencies used in the figure
aud_long <- auditory_variables %>%
...
df %>%
  group_by(HearGroup) %>%
  summarise(
    Audiogram_mean = mean(Audiogram_PCA),
    Audiogram_sd = sd(Audiogram_PCA),
    
    Glimpsing_mean = mean(Glimpsing),
    Glimpsing_sd = sd(Glimpsing),
    
    WM_mean = mean(WorkingMemory_PCA),
    WM_sd = sd(WorkingMemory_PCA),
...
To complement the descriptive statistics, violin plots will be created to compare the distributions of key predictor variables between ONH and OHI groups. The selected variables include:

-   Audiogram PCA (hearing sensitivity)
-   Speech glimpsing ability
-   Working memory PCA
-   Vocabulary knowledge

These visualizations are inspired by Fig. 3 in the original study and provide a clear representation of group differences, including distribution shape, variability, and central tendency.

### Replication Analysis 3: Inferential Analysis: Predictor Importance

The second analysis aims to examine the relative contribution of predictor variables to speech recognition performance. The original study used dominance analysis to quantify the proportion of variance explained by each predictor across datasets and performance levels.
